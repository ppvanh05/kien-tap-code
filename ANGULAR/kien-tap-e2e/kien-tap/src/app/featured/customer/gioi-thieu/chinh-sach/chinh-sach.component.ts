...
const FALLBACK_POLICIES: PolicySection[] = [
  {
    id: 'payment',
...
export class ChinhSachComponent implements OnInit {
  searchText: string = '';
  activeTab: string = FALLBACK_POLICIES[0].id;

  policies: PolicySection[] = [...FALLBACK_POLICIES];

  constructor(private chinhSachService: ChinhSachService) {}

  ngOnInit(): void {
    this.loadPoliciesFromBackend();
  }

  private loadPoliciesFromBackend(): void {
    forkJoin({
      chinhSach: this.chinhSachService.getPublicChinhSach().pipe(
        catchError((err) => {
          console.warn('[ChinhSach] Không tải được CHINH_SACH, dùng fallback:', err);
          return of([]);
        }),
      ),
      huyVe: this.chinhSachService.getPublicChinhSachHuyVe().pipe(
        catchError((err) => {
          console.warn('[ChinhSach] Không tải được CHINH_SACH_HUY_VE:', err);
          return of([]);
        }),
      ),
    }).subscribe(({ chinhSach, huyVe }) => {
      const mapped: PolicySection[] = [
        ...chinhSach.map((item) => this.mapApiPolicy(item)),
        ...huyVe.map((item) => this.mapApiPolicy(item)),
      ];

      if (mapped.length === 0) {
        console.warn(
          '[ChinhSach] API trả về rỗng — có thể do TrangThai = NgungApDung (vd: CS100001). Giữ dữ liệu fallback.',
        );
        return;
      }

      this.policies = mapped;
      this.activeTab = mapped[0].id;
    });
  }

  private mapApiPolicy(item: any): PolicySection {
    return {
      id: item.id,
      title: item.title,
